...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument('--img_save_path', type=str, default='../snapshot/pretrain/')
        self.parser.add_argument('--weight_save_path', type=str, default='../weight/pretrain/')
        ...
        self.parser.add_argument("--log_path", default='./log/pretrain.log')
...
if __name__ == '__main__':
    ...
    dataset = Pretrain_Dataset(img_size=opt.img_size, image_root=opt.data_root, modality='all', augment=opt.augment)
    dataloader = GeneratorDataset(source=dataset, column_names=['img', 'edge_img'])
    batch_size = opt.batch_size
    dataloader = dataloader.batch(batch_size)
    os.makedirs(opt.img_save_path, exist_ok=True)
    os.makedirs(opt.weight_save_path, exist_ok=True)

    logging.basicConfig(filename=opt.weight_save_path + 'pretrain.log',
                        format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
                        level=logging.INFO, filemode='a', datefmt='%Y-%m-%d %I:%M:%S %p')
    model = EdgeMAE(image_size=opt.img_size[0], patch_size=opt.patch_size, in_channels=1, embed_dim=384, depth=12, num_heads=12,
    ...
    for epoch in range(0,opt.epoch):
        ...
        for batch in dataloader.create_dict_iterator():
            ...
            if iteration % opt.save_output == 0:
                ...
                logging.info("[Epoch %d/%d] [Batch %d] [loss: %f]" % (epoch, opt.epoch, iteration, loss))

        if epoch % opt.save_weight == 0:
            mindspore.save_checkpoint(model, opt.weight_save_path + 'EdgeMAE_' + str(epoch) + '.ckpt')
    mindspore.save_checkpoint(model, opt.weight_save_path + "EdgeMAE.ckpt")
